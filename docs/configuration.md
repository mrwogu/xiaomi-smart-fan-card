...
| `visual.graphic_style`         | `blades` | `blades` draws four wide blades, `prop` a caged three-blade propeller, `turbine` a dense nine-blade wheel with a faster spin, `minimal` a bare breathing hub. Ambient styles without a rotor: `stream` (flowing wind lines), `drift` (carried particles), `bars` (airflow equalizer), `plume` (rising thermal column). |
